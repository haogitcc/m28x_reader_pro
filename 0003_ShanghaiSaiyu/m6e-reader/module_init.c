...
int module_init()
{
	...
	//checkerr(rp, ret, 1, "setting fastsearch");
	...
	/*TMR_GEN2_LinkFrequency blf = TMR_GEN2_LINKFREQUENCY_640KHZ;
	...
	TMR_GEN2_TagEncoding tagen = TMR_GEN2_FM0;
	ret = TMR_paramSet(rp, TMR_PARAM_GEN2_TAGENCODING, &tagen);*/
	...
	//ret = TMR_paramSet(rp, TMR_PARAM_TAGREADDATA_UNIQUEBYDATA, &val);
	//checkerr(rp, ret, 1, "setting uniquebydata");
	printf("module init\n");
...
int read_tag()
{
	...
	ret = TMR_read(rp, 20, &count);
	checkerr(rp, ret, 1, "TMR_read");
	printf("read_tag 111111111111111 count : %d\n", count);


	if(0 == count || 2 == count) {
		tag_status_set(count);
		return 0;
	}
  	while (TMR_SUCCESS == TMR_hasMoreTags(rp))
 	{
		...
		checkerr(rp, ret, 1, "TMR_getNextTag");
		TMR_bytesToHex(trd.tag.epc, trd.tag.epcByteCount, epcStr);
		printf("EPC:%s \n", epcStr);
		i++;

		if(1 == i)
			enqueue_p(epcStr, dataStr, trd.tag.epcByteCount * 2);
  	}
