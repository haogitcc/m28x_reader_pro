...
void check_status()
{
	...
	while(1) {
		...
		printf("check_status paper 1 : %ld , %ld\n", tv2.tv_sec, tv2.tv_usec/1000);*/
		tag_status_get(&value);
		socket_data_get(&isSend);
		if(1 == value) {
			gpo2_write();
			tag = dequeue_p();
			//if(isSend) {
				char str[512];
				sprintf(str, "%s,%s", tag->tid, tag->epc);
				tcp_sendBytes(strlen(str), str);
			//}
			free(tag);
		} else {
			//if(isSend) {
...
/*		printf("check_status 11111111111111\n");
		if(tag == NULL && isSend) {
			printf("check_status 22222222222\n");
			if(0 == value) {			
				char * str = "No Tag";
				tcp_sendBytes(strlen(str), str);
		...
		if(tag != NULL) {
			printf("check_status 333333333333\n");
			gpo2_write();
			if(isSend) {
				char str[512];
				sprintf(str, "%s,%s", tag->tid, tag->epc);
...
void socket_data_set(int status)
{
	mid_mutex_lock(g_mutex);
	printf("socket_data_set = %d\n",status);
	isData = status;
	mid_mutex_unlock(g_mutex);
	return 0;
}

void socket_data_get(int *status)
{
	mid_mutex_lock(g_mutex);
	
	//printf("socket_data_get = %d\n",isData);
	*status = isData;
	mid_mutex_unlock(g_mutex);
	return 0;
}
...
void tag_status_set(int value)
{
	...
	printf("tag_status_set paper 1 : %ld , %ld\n", tv.tv_sec, tv.tv_usec/1000);*/
	return 0;	
}

void tag_status_get(int *value)
{
	pthread_mutex_lock(g_mutex);
	*value = status;
	pthread_mutex_unlock(g_mutex);
