...
int sys_config_init(void)
{
	...
	sys_cfg_inset_string("system.epc", &sysConfigs.epc,STRING_LEN_64);
	sys_cfg_inset_int("system.id", &sysConfigs.id);
	sys_cfg_inset_int("system.operation", &sysConfigs.operation);
	sys_cfg_inset_int("system.serial", &sysConfigs.serial);
	sys_cfg_inset_int("system.count1", &sysConfigs.count1);
	sys_cfg_inset_int("system.count2", &sysConfigs.count2);
	sys_cfg_inset_int("system.flags", &sysConfigs.flags);
...
void sys_config_load(int reset)
{
	...
	sysConfigs.target = 0;
	strcpy(sysConfigs.epc, "0000");
	sysConfigs.id = 4;
	sysConfigs.operation = 0;
	sysConfigs.serial = 0;
	sysConfigs.count1 = 0;
	sysConfigs.count2 = 0;
	sysConfigs.flags = 0;
...
int sysSettingGetString(const char* name, char* value, int valueLen, int searchFlag)
{
   ...
   if (!strcmp(name, "ip"))
	...
	else if(!strcmp(name, "epc"))
		strncpy(value, sysConfigs.epc, valueLen);
    else
...
int sysSettingGetInt(const char* name, int* value, int searchFlag)
{
    ...
    if (!strcmp(name, "nettype"))
		...
		*value = sysConfigs.operation;
	else if (!strcmp(name, "serial"))
		*value = sysConfigs.serial;
	else if (!strcmp(name, "count1"))
		*value = sysConfigs.count1;
	else if (!strcmp(name, "count2"))
		*value = sysConfigs.count2;
	else if (!strcmp(name, "flags"))
		*value = sysConfigs.flags;
...
int sysSettingSetString(const char* name, const char* value)
{
    ...
    if (!strcmp(name, "ip")) {
       ...
       if(strcmp(sysConfigs.dns, value)) {
    	...
    	if(strcmp(sysConfigs.epc, value)) {
			strncpy(sysConfigs.epc, value, STRING_LEN_64);
		}
   } else {
...
int sysSettingSetInt(const char* name, const int value)
{
    ...
    if (!strcmp(name, "nettype")) {
		...
		if(sysConfigs.targencoding!= value) 
			...
			if(sysConfigs.id!= value) 
				sysConfigs.id = value;
	} else if (!strcmp(name, "operation")) {
			if(sysConfigs.operation!= value) 
				sysConfigs.operation = value;
	} else if (!strcmp(name, "serial")) {
			if(sysConfigs.serial!= value) 
				sysConfigs.serial = value;
	} else if (!strcmp(name, "count1")) {
			if(sysConfigs.count1!= value) 
				sysConfigs.count1 = value;
	} else if (!strcmp(name, "count2")) {
			if(sysConfigs.count2 != value) 
				sysConfigs.count2 = value;
	} else if (!strcmp(name, "flags")) {
			if(sysConfigs.flags!= value) 
				sysConfigs.flags = value;
	}else if(!strcmp(name, "time")) {
