...
callback(TMR_Reader *reader, const TMR_TagReadData *t, void *cookie)
{
	...
	//printf("epcStr{%s,[%d}]},{e2806894,[%d]}\n", epcStr, strlen(epcStr),strlen(startWith));

	//compare epcStr
	char startWith[128] = "E2806894";
	if(strlen(epcStr)>strlen(startWith))
	{
		if(strncmp(epcStr, startWith, strlen(startWith)) == 0)
		{
		    printf("epc:%s, ant: %d, RSSI: %d\n", epcStr, t->antenna, t->rssi);
	        printf("epcStr{%s,[%d}]},{e2806894,[%d]}\n", epcStr, strlen(epcStr),strlen(startWith));
            int isBuzzer = 0;
			...
			if(0 == isBuzzer) {
				...
				gpio_write_60_v("1");
				mid_timer_create(3, 1, (mid_timer_f)gpio_stop, 0);
				printf("chris_test buzzer2222222 ....\n");
			}
			else 
				{
		        printf("chris_test 333333333 ....\n");
			}
		}
	}		
...
void module_startReading()
{
	...
	int isStart,ret, i, readLen, isUser, user_addr, user_length, isFilter;
	uint8_t *antennaList= NULL;
	int readpower = 500;
	...
	while(1) {
		...
		//if(isStart) {
            ...
            isFilter = 0;
			sysSettingGetInt("isFilter", &isFilter, 1);
/*			if(isFilter)
			{
				// 设置筛选条件 start
				...
				mask[0] = 0x1A;
				mask[1] = 0x22;
				TMR_TF_init_gen2_select(pfilter, false, TMR_GEN2_BANK_EPC, bitPointer, maskBitLength, mask);
				//(TMR_TagFilter *filter, bool invert, TMR_GEN2_Bank bank, uint32_t bitPointer, uint16_t maskBitLength, uint8_t *mask)

				TMR_RP_set_filter(&plan, pfilter);
				printf("set filter done ...\n");
				// 设置筛选条件 end
			}
*/
/*
...
*/
            ...
            //SimpleReadPlan plan = new SimpleReadPlan(antennaList, TagProtocol.GEN2, new Select(false, Bank.EPC, 0x20f, 0x01, mask), null, 1000);
