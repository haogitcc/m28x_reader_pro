...
callback(TMR_Reader *reader, const TMR_TagReadData *trd, void *cookie)
{
	...
	if (0 < trd->data.len)
    {
	    ...
	    char filterdata[255];
  		...
  		printf("[%s]'s data is [%s], length is %d byte(s)\n", epcStr, dataStr, trd->data.len);
	    sysSettingGetString("user_data", filterdata, 64, 1);
		printf("#filterdata is [%s]\n", filterdata);
		if(strncmp(filterdata, dataStr, strlen(filterdata)) == 0) { // 相等就报警
			int isBuzzer = 0;
	...
	while(1) {
		...
		//if(isStart) {
			...
			if(isUser) {
			    ...
			    printf("#user_addr: [%d] wordaddr\n", user_addr);
				printf("#user_length: [%d] word(s)\n", user_length);
				
		       	ret = TMR_TagOp_init_GEN2_ReadData(&op, TMR_GEN2_BANK_USER, user_addr, user_length);
      				checkerr(rp, ret, 1, "creating tagop: GEN2 read data");
      				ret = TMR_RP_set_tagop(&plan, &op);
      				checkerr(rp, ret, 1, "setting tagop");
					printf("set read user op success\n");
			} else {
				ret = TMR_RP_set_tagop(&plan, NULL);
      				checkerr(rp, ret, 1, "setting tagop");
			}
			...
			{
				// 设置筛选条件 start
				TMR_TagFilter setfilter, *pfilter = &setfilter;
				uint32_t bitPointer;
