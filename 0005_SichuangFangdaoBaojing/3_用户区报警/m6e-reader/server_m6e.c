...
int m6e_user_reponse(int flags)
{
	uint8_t msg[TMR_SR_MAX_PACKET_SIZE];
	uint8_t i;
	uint16_t crc;
	...
	if(0 == flags) {
		int readpower;
		sysSettingGetString("user_data", filter, 64, 1);
		sysSettingGetInt("user_addr", &addr, 1);
	       int len = (int)(strspn(filter, hexChars) / 2);
             epc = malloc(len);
		TMR_hexToBytes(filter, epc, len, NULL);
		printf("filter length: %d , %d\n",strlen(filter), strlen(epc));
...
int m6e_user_set(char *buffer)
{
	char filter[64];
	char epcStr[64];
	char dataStr[64];
	int i = 1;
	uint8_t len = GETU8(buffer, i);
	i = 3;
	//uint16_t len = GETU8(buffer,i);
	uint8_t isUser = GETU8(buffer, i);
	memset(filter , 0, 64);
	memset(epcStr , 0, 64);
	memset(dataStr , 0, 64);
	if(isUser) {
		uint8_t addr = GETU8(buffer, i);
		uint8_t data_len = GETU8(buffer, i);
		int filter_len = len - 3;
		strncpy(filter, buffer + 6, filter_len);
		//strncpy(filter, buffer + 4, len);
		TMR_bytesToHex(filter, filter_len, epcStr);
		sysSettingSetInt("user_addr", addr);
		strncpy(dataStr, epcStr, data_len);
		printf("epcstr : %s\n", epcStr);
		sysSettingSetString("user_data", dataStr);
	
		int user_len1 = data_len % 8;
		int user_len2 = data_len / 8;
		printf("user_len1 : %d, user_len2 : %d\n", user_len1, user_len2);
		if(0 == user_len1 && 0 == user_len2)
			sysSettingSetInt("user_length", 1);
		else if(0 == user_len1 && 0 != user_len2) 
			sysSettingSetInt("user_length", user_len2);
		else if(0 != user_len1 && 0 != user_len2)
			sysSettingSetInt("user_length", user_len2 + 1);
		else if(0 != user_len1 && 0 == user_len2)
			sysSettingSetInt("user_length", 1);
	}

		sysSettingSetInt("isUser", isUser);
		m6e_user_reponse(1);
		sys_config_save();
		return;	
...
static void*m6e_pthread_client(void *arg)
{
	...
	while(1) {
        ...
        if(FD_ISSET(connected_fd, &set) || FD_ISSET(connected_fd, &errornew_fd)) {
			...
			printf("buffer = %p,%p,%p,%p,%p, %p, %p, %d\n",buffer[0],buffer[1],buffer[2],buffer[3],buffer[4],buffer[5],buffer[6],error);
			if(error == 0)
				break;
